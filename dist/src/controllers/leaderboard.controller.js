...
const getLeaderboard = async (req, res, next) => {
    ...
    try {
        ...
        const leaderboard = await (0, leaderboard_service_1.getLeaderboardService)(gameId, parsedPage, parsedLimit);
...
const postScore = async (req, res, next) => {
    ...
    try {
        ...
        const updatedScore = await (0, leaderboard_service_1.postScoreService)(gameId, score, userId);
...
const getFriendLeaderboard = async (req, res, next) => {
    ...
    try {
        ...
        const leaderboard = await (0, leaderboard_service_1.getFriendLeaderboardService)(gameId, parsedPage, parsedLimit, userId);
        res.json(leaderboard);
    }
    catch (err) {
        next(err);
...
const getUserScore = async (req, res, next) => {
    ...
    try {
        if (!userId)
            throw new AppError_1.AppError("Unauthorized", 401);
        const data = await (0, leaderboard_service_1.getUserScoreService)(gameId, userId);
        res.json(data);
    }
    catch (err) {
        next(err);
...
const getUserRank = async (req, res, next) => {
    ...
    try {
        if (!userId)
            throw new AppError_1.AppError("Unauthorized", 401);
        const rankData = await (0, leaderboard_service_1.getUserRankService)(gameId, userId);
        res.json(rankData);
    }
    catch (err) {
        next(err);
