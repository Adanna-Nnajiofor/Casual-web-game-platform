...
const getLeaderboardService = async (gameId, page = 1, limit = 10, userId) => {
    ...
    const result = await Leaderboard_model_1.default.aggregate(pipeline);
    if (!result.length) {
...
const postScoreService = async (gameId, score, userId) => {
    if (!mongoose_1.default.Types.ObjectId.isValid(gameId)) {
        throw new AppError_1.AppError("Invalid game ID", 400);
    }
    ...
    return await Leaderboard_model_1.default.findOneAndUpdate({ gameId, userId }, { $max: { score } }, { upsert: true, new: true });
};
exports.postScoreService = postScoreService;
const getFriendLeaderboardService = async (gameId, page, limit, userId) => {
    if (!mongoose_1.default.Types.ObjectId.isValid(gameId)) {
        throw new AppError_1.AppError("Invalid game ID", 400);
    }
    ...
    if (friendsList.length === 0)
        return [];
    return await Leaderboard_model_1.default.find({ gameId, userId: { $in: friendsList } })
...
const getUserScoreService = async (gameId, userId) => {
    if (!mongoose_1.default.Types.ObjectId.isValid(gameId)) {
        throw new AppError_1.AppError("Invalid game ID", 400);
    }
    const entry = await Leaderboard_model_1.default.findOne({ gameId, userId })
        .populate("userId", "username")
        .select("score userId");
...
const getUserRankService = async (gameId, userId) => {
    if (!mongoose_1.default.Types.ObjectId.isValid(gameId)) {
        throw new AppError_1.AppError("Invalid game ID", 400);
    }
